...
var textpattern = (function () {
  ...
  var $_8d9xtprtjlnuefpp = { getPatterns: getPatterns };
  ...
  var patternFromRng = function (patterns, rng, space) {
    ...
    var endPattern = $_ej9v72s1jlnuefq7.findEndPattern(patterns, text, rng.startOffset, delta);
    ...
    var startPattern = $_ej9v72s1jlnuefq7.findPattern(patterns, patternRng.toString());
  ...
  var applyBlockFormat = function (editor, patterns) {
    ...
    if (textBlockElm) {
      ...
      if (firstTextNode) {
        pattern = $_ej9v72s1jlnuefq7.findPattern(patterns, firstTextNode.data);
        if (!pattern) {
          return;
        }
  ...
  function handleEnter(editor, patterns) {
    ...
    wrappedTextNode = $_37qh0vryjlnuefq1.applyInlineFormatEnter(editor, patterns);
    if (wrappedTextNode) {
    ...
    $_37qh0vryjlnuefq1.applyBlockFormat(editor, patterns);
  }
  function handleInlineKey(editor, patterns) {
    var wrappedTextNode, lastChar, lastCharNode, rng, dom;
    wrappedTextNode = $_37qh0vryjlnuefq1.applyInlineFormatSpace(editor, patterns);
    if (wrappedTextNode) {
      dom = editor.dom;
  ...
  var setup = function (editor, patternsState) {
    ...
    editor.on('keydown', function (e) {
      if (e.keyCode === 13 && !global$2.modifierPressed(e)) {
        $_dxhrr1rxjlnuefpw.handleEnter(editor, patternsState.get());
      }
    }, true);
    editor.on('keyup', function (e) {
      if ($_dxhrr1rxjlnuefpw.checkKeyCode(keyCodes, e)) {
        $_dxhrr1rxjlnuefpw.handleInlineKey(editor, patternsState.get());
      }
    });
    editor.on('keypress', function (e) {
      if ($_dxhrr1rxjlnuefpw.checkCharCode(charCodes, e)) {
  ...
  var $_ffpiworujlnuefps = { setup: setup };

  global.add('textpattern', function (editor) {
    var patternsState = Cell($_8d9xtprtjlnuefpp.getPatterns(editor.settings));
    $_ffpiworujlnuefps.setup(editor, patternsState);
    return $_3vkhukrsjlnuefpo.get(patternsState);
  });
  function Plugin () {
  }
