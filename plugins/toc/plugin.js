...
var toc = (function () {
  ...
  var $_9uuqxjsbjlnuefs8 = { create: create };

  var tocId = $_9uuqxjsbjlnuefs8.create('mcetoc_');
  ...
  var readHeaders = function (editor) {
    var tocClass = $_a880cssajlnuefs6.getTocClass(editor);
    var headerTag = $_a880cssajlnuefs6.getTocHeader(editor);
    var selector = generateSelector($_a880cssajlnuefs6.getTocDepth(editor));
  ...
  var generateTocHtml = function (editor) {
    ...
    return '<div class="' + editor.dom.encode($_a880cssajlnuefs6.getTocClass(editor)) + '" contenteditable="false">' + html + '</div>';
  };
  var generateTocContentHtml = function (editor) {
    ...
    html += generateTitle($_a880cssajlnuefs6.getTocHeader(editor), global$2.translate('Table of Contents'));
    for (i = 0; i < headers.length; i++) {
      h = headers[i];
      h.element.id = h.id;
  ...
  var register = function (editor) {
    editor.addCommand('mceInsertToc', function () {
      $_elrgs5s6jlnuefs0.insertToc(editor);
    });
    editor.addCommand('mceUpdateToc', function () {
      $_elrgs5s6jlnuefs0.updateToc(editor);
    });
  };
  var $_9367wgs5jlnuefry = { register: register };

  var setup = function (editor) {
    var $ = editor.$, tocClass = $_a880cssajlnuefs6.getTocClass(editor);
  ...
  var $_3bd1gzscjlnuefsa = { setup: setup };

  var toggleState = function (editor) {
    return function (e) {
      var ctrl = e.control;
      editor.on('LoadContent SetContent change', function () {
        ctrl.disabled(editor.readonly || !$_elrgs5s6jlnuefs0.hasHeaders(editor));
      });
    };
  };
  var isToc = function (editor) {
    return function (elm) {
      return elm && editor.dom.is(elm, '.' + $_a880cssajlnuefs6.getTocClass(editor)) && editor.getBody().contains(elm);
    };
  };
  ...
  var $_f4p7lfsdjlnuefsd = { register: register$1 };

  global.add('toc', function (editor) {
    $_9367wgs5jlnuefry.register(editor);
    $_f4p7lfsdjlnuefsd.register(editor);
    $_3bd1gzscjlnuefsa.setup(editor);
  });
  function Plugin () {
  }
